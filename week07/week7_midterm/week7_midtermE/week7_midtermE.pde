...
void draw(){
  ...
  for(int i = 0; i<num; i++){
    for(int j = 0; j<num; j++){
      ...
      mouseY > j*height/num && mouseY <=(j+1)*height/num){ //changed mouseY conditionals to look at j
        opacity[i][j] = 0; 
      }if(opacity[i][j]<255){
        opacity[i][j] ++; 
      }
      fill(255, opacity[i][j]); 
      rect(i*width/num, j*height/num, width/num, height/num); //added *width/num & *height/num to fix spacing
    }
  }
}
